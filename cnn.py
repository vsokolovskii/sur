...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            data, target = data.type(torch.FloatTensor).to(device), target.type(torch.FloatTensor).to(device)
            output = model(data).flatten()
            test_loss += torch.nn.BCELoss()(output, target).item()  # sum up batch loss
...
def main(show_image=False):
    ...
    # create dataloader
    train_loader = torch.utils.data.DataLoader(PngsDataset('pngs-train.csv', transform=transform), batch_size=8, shuffle=True)
    val_loader = torch.utils.data.DataLoader(PngsDataset('pngs-test.csv', transform=transform), batch_size=1, shuffle=False)
